...
\usepackage{times}
\usepackage{graphicx}
\graphicspath{{images/android-studio-setup/}{images/jdk-setup/}{images/genymotion-setup/}{images/}}
\usepackage{subfig}
				...
				\item Nội dung của các file chương trình:
					\begin{itemize}
						\item Nội dung của các file \verb|AndroidMainfest.xml|:
							\lstinputlisting[language=XML]{AndroidManifest-Cube.xml}
			
						\item Nội dung của file \verb|MainActivity.java|:
							\lstinputlisting[language=Java]{MainActivity-Cube.java}
		
						\item Nội dung của file \verb|CubeGLSurfaceView.java|:
							\lstinputlisting[language=Java]{CubeGLSurfaceView.java}
					\end{itemize}
				...
				\item Kết quả khi chạy chương trình: các mặt của khối lập phương được thể hiện trên hình~\ref{Fig:opengles-cube}.
					\begin{figure}[!h]
						\begin{center}
							...
							\hspace{.5cm}
							\includegraphics[scale=.6]{opengles-cube-2.png} 
						\end{center}
						\caption{Vẽ và xoay hình lập phương sử dụng OpenGL ES 2.0 trên Android}
						\label{Fig:opengles-cube}
					\end{figure}								 
			\end{itemize}
		
		\item \textit{Tự động xoay khối lập phương:}
			Nội dung của các file chương trình:
					\begin{itemize}
						\item Source code của project được lưu ở địa chỉ: 
				
				\url{https://github.com/minhdatvnus/nienluan3/blob/master/CubeRotateOpenGLES20.zip}
				
						\item Nội dung của các file \verb|AndroidMainfest.xml|:
							\lstinputlisting[language=XML]{AndroidManifest-Cube-Rotate.xml}
			
						\item Nội dung của file \verb|MainActivity.java|:
							\lstinputlisting[language=Java]{MainActivity-Cube-Rotate.java}
		
						\item Nội dung của file \verb|CubeRotateGLSurfaceView.java|:
							\lstinputlisting[language=Java]{CubeRotateGLSurfaceView.java}
							
						\item Nội dung của file \verb|Cube.java|
							\lstinputlisting[language=Java]{Cube-Rotate.java}
					\end{itemize}
...
\subsection{Vẽ đối tượng là khối cầu}
	Nội dung của các file chương trình:
		\begin{itemize}
			...
			\item Nội dung của các file \verb|AndroidMainfest.xml|:
				\lstinputlisting[language=XML]{AndroidManifest-Circle.xml}
			
			\item Nội dung của file \verb|MainActivity.java|:
				\lstinputlisting[language=Java]{MainActivity-Circle.java}
		
			\item Nội dung của file \verb|CircleOpenGLSurfaceView.java|:
				\lstinputlisting[language=Java]{CircleOpenGLSurfaceView.java}

			\item Nội dung của file \verb|CircleRenderer.java|:
				\lstinputlisting[language=Java]{CircleRenderer.java}
									
			\item Nội dung của file \verb|Sphere.java|:
				\lstinputlisting[language=Java]{Sphere.java}					
		\end{itemize}
